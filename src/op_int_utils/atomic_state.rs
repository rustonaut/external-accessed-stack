...
//!    does itself implement [`OperationInteraction`]. This type is moved into the `RABufferAnchor`
...
impl<Result> Anchor<Result> {
    ...
    /// the underlying `RABufferAnchor`. Everything else would require
...
impl<Result> Completer<Result> {
    ...
    ///   pinned into a `RABufferAnchor` this is already enforced.
